...
$(function () {
    ...
    var dt_product_table = $('.datatables-cities'),
        categoryView = baseUrl + '/admin/api-cities',
        offCanvasForm = $('#offcanvasAddCity'),
        categoryObj = list_countries,
        filterButton = $('#filterByCountryBtn'),
        select2 = $('.select2');
    ...
    if (dt_product_table.length) {
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return (
                            ...
                            `<button class="btn btn-sm btn-icon edit-record" data-id="${full['id']}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddCity"><i class="ti ti-edit"></i></button>` +
        ...
        dt_category.on('init', function () {
            ...
            filterButton.on('click', function () {
                ...
                dt_category.ajax.url(getUpdatedCityViewUrl(selectedCountry)).load();
            });
    ...
    $(document).on('click', '.delete-record', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: `${baseUrl}/admin/cities/${city_id}`,
                    success: function () {
                        dt_category.draw();
                    },
    ...
    $(document).on('click', '.edit-record', function () {
        ...
        $.get(`${baseUrl}/admin/cities\/${city_id}\/edit`, function (data) {
            console.log(data)
            $('#city_id').val(data.id);
            $('#add-city-name-ar').val(data.name.ar);
            $('#add-city-name-en').val(data.name.en);
    ...
    const fv = FormValidation.formValidation(addNewCityForm, {
...
function getUpdatedCityViewUrl(countryId) {
    // Update the URL with the selected category filter
    return baseUrl + '/admin/api-cities?country_id=' + countryId;
}
