...
$(function () {
    ...
    var dt_category_table = $('.datatables-treatments'),
        select2 = $('.select2'),
        sub_select2 = $('.select2_sub'),
        categoryView = baseUrl + '/admin/api-treatments',
        offCanvasForm = $('#offcanvasAddTreatment');
    ...
    if (dt_category_table.length) {
        var dt_category = dt_category_table.DataTable({
            ...
            serverSide: true,
            ajax: {
                url: baseUrl + '/admin/api-treatments'
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return (
                            ...
                            `<button id="editButton" class="btn btn-sm btn-icon edit-record" data-id="${full['id']}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddTreatment"><i class="ti ti-edit"></i></button>` +
            ...
            buttons: [
                ...
                },
                {
                    ...
                    attr: {
                        ...
                        'data-bs-target': '#offcanvasAddTreatment'
                    }
                }
            ],
    ...
    $(document).on('click', '.delete-record', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: `${baseUrl}/admin/treatments/${treatment_id}`,
                    success: function () {
                        dt_category.draw();
                    },
                ...
                Swal.fire({
                    title: cancel,
                    text: 'The Treatment is not deleted!',
                    icon: 'error',
                    customClass: {
                        confirmButton: 'btn btn-success'
                    }
    ...
    $('.add-new').on('click', function () {
        $('#treatment_id').val(''); //reseting input field
        $('#offcanvasAddTreatmentLabel').html(addNewTranslation);
    });
    ...
    const addNewTreatmentForm = document.getElementById('addNewTreatmentForm');

    // category form validation
    const fv = FormValidation.formValidation(addNewTreatmentForm, {
        fields: {
            name_en: {
                validators: {
                    notEmpty: {
